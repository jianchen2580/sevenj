...
var message = require('../services/notification');
...
exports.index = function (req, res, next) {
  ...
  User.getUserByName(user_name, function (err, user) {
    ...
    var ep = EventProxy.create('recent_topics', 'recent_replies', 'relation', render);

    var query = {author_id: user._id};
    var opt = {limit: 5, sort: [['create_at', 'desc']]};

    Topic.getTopicsByQuery(query, opt, ep.done('recent_topics'));
    ...
    Reply.getRepliesByAuthorId(user._id, ep.done(function (replies) {
      ...
      var opt = {limit: 5, sort: [['create_at', 'desc']]};
      Topic.getTopicsByQuery(query, opt, ep.done('recent_replies'));
    }));

    if (!req.session.user) {
      ep.emit('relation', null);
    } else {
      Relation.getRelation(req.session.user._id, user._id, ep.done('relation'));
    }
  });
};
