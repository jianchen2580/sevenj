...
exports.signup = function (req, res, next) {
  ...
  name = sanitize(name).str;
  var loginname = name.toLowerCase();
  var pass = sanitize(req.body.password).trim();
  pass = sanitize(pass).str;
  var email = sanitize(req.body.email).trim();
  email = sanitize(email).str;
  email = email.toLowerCase();
  var re_pass = sanitize(req.body.password2).trim();
  re_pass = sanitize(re_pass).str;

  if (name === '' || pass === '' || re_pass === '' || email === '') {
    res.render('account/signup', {error: '信息不完整。', name: name, email: email});
    return;
  }

  if (name.length < 5) {
    res.render('account/signup', {error: '用户名至少需要5个字符。', name: name, email: email});
    return;
  }
  ...
  try {
    ...
    res.render('account/signup', {error: e.message, name: name, email: email});
    return;
  }

  if (pass !== re_pass) {
    res.render('account/signup', {error: '两次密码输入不一致。', name: name, email: email});
    return;
  }
  ...
  User.getUsersByQuery({'$or': [{'loginname': loginname}, {'email': email}]}, {}, function (err, users) {
    ...
    if (users.length > 0) {
      res.render('account/signup', {error: '用户名或邮箱已被使用。', name: name, email: email});
      return;
    }
...
exports.signin = function (req, res, next) {
  var loginname = sanitize(req.body.email).str.trim().toLowerCase();
  var pass = sanitize(req.body.password).str.trim();

  if (!loginname || !pass) {
    return res.render('account/signin', { error: '信息不完整。' });
  }
  ...
  User.getUserByMail(loginname, function (err, user) {
    ...
    if (!user) {
      return res.render('account/signin', { error: '这个用户不存在。' });
    }
    pass = md5(pass);
    if (pass !== user.pass) {
      return res.render('account/signin', { error: '密码错误。' });
    }
    if (!user.active) {
      // 从新发送激活邮件
      mail.sendActiveMail(user.email, md5(user.email + config.session_secret), user.name, user.email);
      return res.render('account/signin', { error: '此帐号还没有被激活，激活链接已发送到 ' + user.email + ' 邮箱，请查收。' });
    }
...
exports.showSearchPass = function (req, res) {
  res.render('account/search_pass');
};
...
exports.updateSearchPass = function (req, res, next) {
  ...
  try {
    ...
    res.render('account/search_pass', {error: e.message, email: email});
    return;
  }
  ...
  User.getUserByMail(email, function (err, user) {
    if (!user) {
      res.render('account/search_pass', {error: '没有这个电子邮箱。', email: email});
      return;
    }
...
exports.reset_pass = function (req, res, next) {
  ...
  User.getUserByQuery(name, key, function (err, user) {
    ...
    return res.render('account/reset', {name : name, key : key});
  });
};
...
exports.update_pass = function (req, res, next) {
  ...
  if (psw !== repsw) {
    return res.render('account/reset', {name : name, key : key, error : '两次密码输入不一致。'});
  }
  User.getUserByQuery(name, key, function (err, user) {
    if (err) {
      return next(err);
